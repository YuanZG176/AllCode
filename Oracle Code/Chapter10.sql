...
select address, replace(address,'P.0.','POST OFFICE')
from customers
where state = 'FL';
...
from orders
where orderdate  = to_date('3月 31,2009','Month DD, YYYY');
...
select order#,orderdate,shipdate,
nvl(shipdate,'06-4月-09')-orderdate "Ship Days"
from orders
where orderdate >= '03-4月-09';
...
and o.customer# = oi.order#
and oi.isbn = b.isbn
and c.lastname='LUCAS' and c.firstname='JAKE'
order by o.orderdate asc, profit desc;
...
select P.gift,b.retail
from bookS b,promotion P
where b.retail between p.minretail and maxretail
